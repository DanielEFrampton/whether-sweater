    ...
    describe 'I receive a JSON:API response' do
      describe 'with weather data about current conditions, including' do
        it 'current weather condition, matching icon, and current temperature' do
          expect(@data['data']['attributes']['currently']['summary']). to eq(@expected['data']['attributes']['currently']['summary'])
          expect(@data['data']['attributes']['currently']['summaryIcon']). to eq(@expected['data']['attributes']['currently']['summaryIcon'])
          expect(@data['data']['attributes']['currently']['formattedTemperature']). to eq(@expected['data']['attributes']['currently']['formattedTemperature'])
        end

        it 'feels like, humidity, visability, uv index and uv exposure category' do
          expect(@data['data']['attributes']['currently']['formattedFeelsLike']). to eq(@expected['data']['attributes']['currently']['formattedFeelsLike'])
          expect(@data['data']['attributes']['currently']['formattedHumidity']). to eq(@expected['data']['attributes']['currently']['formattedHumidity'])
          expect(@data['data']['attributes']['currently']['formattedVisibility']). to eq(@expected['data']['attributes']['currently']['formattedVisibility'])
          expect(@data['data']['attributes']['currently']['uvIndex']). to eq(@expected['data']['attributes']['currently']['uvIndex'])
          expect(@data['data']['attributes']['currently']['uvExposureCategory']). to eq(@expected['data']['attributes']['currently']['uvExposureCategory'])
        end

        it 'daily high and low, daily summary, and overnight summary' do
          expect(@data['data']['attributes']['currently']['dayTemperatureLow']). to eq(@expected['data']['attributes']['currently']['dayTemperatureLow'])
          expect(@data['data']['attributes']['currently']['dayTemperatureHigh']). to eq(@expected['data']['attributes']['currently']['dayTemperatureHigh'])
          expect(@data['data']['attributes']['currently']['dayTemperatureHigh']). to eq(@expected['data']['attributes']['currently']['dayTemperatureHigh'])
          expect(@data['data']['attributes']['currently']['todaySummary']). to eq(@expected['data']['attributes']['currently']['todaySummary'])
          expect(@data['data']['attributes']['currently']['next24HoursSummary']). to eq(@expected['data']['attributes']['currently']['next24HoursSummary'])
        end
      end

      it 'with data about city, state, country, time, and date' do
        require "pry"; binding.pry
        expect(@data['data']['attributes']['location']['formattedCityState']).to eq(@expected['data']['attributes']['location']['formattedCityState'])
        expect(@data['data']['attributes']['location']['country']).to eq(@expected['data']['attributes']['location']['country'])
        expect(@data['data']['attributes']['dateTime']['formattedDateTime']).to eq(@expected['data']['attributes']['dateTime']['formattedDateTime'])
      end
      ...
      it 'with hourly temperature forecast for next 8 hours' do
        @data['data']['attributes']['hourlyForecasts'].each_with_index do |hourly, index|
          ...
          expect(hourly['summaryIcon']).to eq(@expected['data']['attributes']['hourlyForecasts'][index]['summaryIcon'])
        end
      end
      ...
      it 'with weather summary, icon, high and low temps data for next 5 days' do
        @data['data']['attributes']['dailyForecasts'].each_with_index do |daily, index|
          ...
          expect(daily['summaryIcon']).to eq(@expected['data']['attributes']['dailyForecasts'][index]['summaryIcon'])
